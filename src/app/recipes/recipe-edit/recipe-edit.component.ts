...
export class RecipeEditComponent implements OnInit, OnChanges {
  ...
  initForm(): void {
    ...
    const ingredients = (this.recipe?.ingredients || []).map(({ name, amount }) => this.getNewIngredientFormGroup(name, amount));

    this.recipeFormGroup = new FormGroup({
      name: new FormControl(this.recipe?.name || null, Validators.required),
      imagePath: new FormControl(this.recipe?.imagePath || null, Validators.required),
      description: new FormControl(this.recipe?.description || null, Validators.required),
  ...
  private getNewIngredientFormGroup(name: string | null, amount: number | null) {
  ...
  onSubmit() {
    if(this.mode === 'edit') {
      this.recipesService.updateRecipe(this.recipeIndex, {...this.recipeFormGroup.value, id: this.id})
    } else {
      const newId = this.recipesService.getNextId();
      this.recipesService.addRecipe({...this.recipeFormGroup.value, id: newId });
      this.id = newId;
    }
